...
rule sourmash_sketch_microbial:
	...
	shell:
		...
		grep "^>" {input.derreplicated_microbial_contigs} | sed 's/^>//' | awk -v dir="{input.derreplicated_microbial_contigs_dir}/combined_microbial_derreplicated_tot.part_" '{{print $1 ", combined_microbial_derreplicated_tot.part" dir $1 ".fasta,"}}' >> {output.manysketch_csv}
