...
my $tag_fna=0;
my $threads=1;
GetOptions ('help' => \$h, 'h' => \$h, 'f=s'=>\$fasta_file, 'i=s'=>\$th_id, 'c=s'=>\$th_cover, 'nofna'=>\$tag_fna, 't=s'=>\$threads);
...
my $mummer_file_delta=$root."-nucmer.out.delta";
...
if (-e $mummer_file_out){
        ...
        $cmd="$nucmer_path --maxmatch --nooptimize $fasta_file $fasta_file -p $mummer_file -t $threads; show-coords $mummer_file_delta > $mummer_file_out";
